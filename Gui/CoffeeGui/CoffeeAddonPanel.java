...
import Gui.MainGui.*;
import Gui.CustomGui.SegmentedTab; // CHANGED: ใช้ segmented tab
...
public class CoffeeAddonPanel extends JPanel {
    ...
    private SegmentedTab hotToggle, icedToggle; // CHANGED
    ...
    public CoffeeAddonPanel(MaeveCoffeeUI ui) {
        ...
        // LEFT : baseline "ADDON" + drink name (Cappuccino)
        ...
        // RIGHT : compact Total badge (short & not stretched)
        ...
        typeDivider.setBackground(new Color(220, 220, 220));
        ...
        // วางก้อน segmented ทางขวา และคั่นกลางเพื่อคุมช่องไฟ
        typeRow.add(Box.createHorizontalStrut(24), BorderLayout.CENTER);
        typeRow.add(typeStrip, BorderLayout.EAST);

        content.add(typeRow);
        content.add(Box.createVerticalStrut(32)); // ระยะแนวตั้งตามภาพ
        ...
        // เส้นแบ่งซ้าย/ขวาของกลาง
        JPanel divLeft = new JPanel();
        divLeft.setPreferredSize(new Dimension(1, 32));
        divLeft.setBackground(new Color(220, 220, 220));
        JPanel divRight = new JPanel();
        divRight.setPreferredSize(new Dimension(1, 32));
        divRight.setBackground(new Color(220, 220, 220));
    ...
    private JButton createStepButton(String text, boolean roundLeft, boolean roundRight, int w, int h) {
        JButton b = new JButton(text) {
            ...
            protected void paintComponent(Graphics g) {
                ...
                int tx = (ww - fm.stringWidth(getText())) / 2;
